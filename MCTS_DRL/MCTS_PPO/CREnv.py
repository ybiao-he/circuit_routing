...
class CREnv_MCTS():
    def __init__(self):
        super(CREnv_MCTS, self).__init__()
    ...
    def takeAction(self, action):
        ...
        newState.action_node_pre = newState.action_node

        newState.path_length += 1

        # pre-determine new action node
        newState.action_node = (newState.action_node[0]+action_tmp[0], newState.action_node[1]+action_tmp[1])
        # check/adjust new action node and set its value
        x = newState.action_node[0]
        y = newState.action_node[1]

        if 0 <= x < newState.board.shape[0] and 0 <= y < newState.board.shape[1]:
            if newState.action_node == newState.finish[newState.pairs_idx]:
                newState.goto_new_net(True)
            elif newState.board[newState.action_node]!=0:
                newState.collide = True
                newState.goto_new_net(False)
            else:
                newState.board[newState.action_node_pre] = 1
        ...
        return newState, newState.isTerminal()
