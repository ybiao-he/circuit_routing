...
class Policy(PolicyBase):
    ...
    def update(self, rollouts):
        ...
        #     v_losses = self._inner_update_loop( rollouts['vec_obses'], 
        #                                         rollouts['vis_obses'], 
        #                                         rollouts['actions'], 
        #                                         rollouts['advs'], 
        #                                         rollouts['returns'], 
        ...
        # losses['v_loss'] = v_losses['v_loss']
        return losses                                                                   # Return Metrics
        
    def _inner_update_loop(self, vec_obses, vis_obses, actions, advs, returns, logp_t, inds, loss_type):
        """
    ...
    def _loss(self, vec_obs, vis_obs, logp_old, act, adv, returns):
        ...
        pi, values = self.model.predict({"vec_obs": vec_obs, "vis_obs": vis_obs})               # output from neural network are logits or mu 
    ...
    # @tf.function
    def _train_one_step(self, vec_obs, vis_obs, act, adv, logp_old, returns, loss_type):
