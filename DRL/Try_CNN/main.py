...
def gen_train_boards(board, paths):
    ...
    for path in paths:
        ...
        for i in range(path_lenght-1):
            ...
            board[path[i]] = max_value
            tem_board = np.ravel(board)/max_value
            action = (path[i+1][0]-path[i][0], path[i+1][1]-path[i][1])
            action_idx = direction_map[action]
            tem_board = np.append(tem_board, [path_idx, action_idx])
...
if __name__== "__main__":
    ...
    if gen_raw_board:
        ...
        dataset = np.array(dataset)
        np.savetxt(os.path.join(saved_path, 'training_data.csv'), dataset, delimiter=',')
